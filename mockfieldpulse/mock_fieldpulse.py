...
Data:
  Stored in PostgreSQL. Connection URL comes from DATABASE_URL (loaded from
  voice-intake/.env), defaulting to postgresql://localhost:5432/fieldpulse_dev.
  Tables are created idempotently on startup and the realistic kitchen/bath
  seed only fires when the customers table is empty — restarts neither wipe
  nor duplicate data. The previous JSON-file backend lives in
  mock_fieldpulse_json_backup.py.
"""
...
# ─────────────────────────────────────────────────────────────────────
# Storage — PostgreSQL via psycopg 3.
# The previous JSON-file backend lives in mock_fieldpulse_json_backup.py.
# ─────────────────────────────────────────────────────────────────────

# Pick up DATABASE_URL from voice-intake/.env (the project's single .env).
_VOICE_INTAKE_ENV = Path(__file__).resolve().parent / "voice-intake" / ".env"
load_dotenv(_VOICE_INTAKE_ENV)

DATABASE_URL = os.environ.get(
    "DATABASE_URL", "postgresql://localhost:5432/fieldpulse_dev"
)

# One pool, opened at startup, closed at shutdown. Each request grabs a
# short-lived connection via `with pool.connection() as conn:`.
# psycopg honors `?sslmode=require` from the URL (Render-style URLs) and
# defaults to no SSL locally — no special-casing needed.
pool = ConnectionPool(
    DATABASE_URL,
    min_size=1,
    max_size=10,
...
def _now():
    """Kept for backward compatibility — most timestamps now come from
    Postgres `now()` defaults instead."""
...
# ─────────────────────────────────────────────────────────────────────
# Database init, seed, and reset
# ─────────────────────────────────────────────────────────────────────

# Schema — created idempotently at startup. JSONB on `address` and
# `custom_fields` preserves the nested shapes the API used to return out
# of the JSON file. ON DELETE CASCADE on the FKs gives us the cascade-
# delete-on-customer-DELETE behavior automatically.
_SCHEMA_SQL = """
...
def init_db():
    """Create tables if missing; seed only when customers is empty.

    Idempotent — safe to call on every startup. Restarts neither wipe
    nor duplicate data.
    """
    with pool.connection() as conn:
        ...
        if count == 0:
            print(f"  seeding (customers table is empty)")
            _seed_into_db(conn, _seed_data())
        else:
            print(f"  database has {count} customer(s) — skipping seed")
        conn.commit()


def _seed_into_db(conn, seed):
    """Translate the readable seed dict into INSERTs."""
    with conn.cursor() as cur:
        for c in seed["customers"]:
            cur.execute(
                """
                INSERT INTO customers
                    (id, display_name, first_name, last_name, email, phone, address)
                VALUES
                    (%s, %s, %s, %s, %s, %s, %s)
                """,
                [
                    c["id"], c.get("display_name"), c.get("first_name"),
                    c.get("last_name"), c.get("email"), c.get("phone"),
                    Jsonb(c.get("address") or {}),
                ],
            )
        for j in seed["jobs"]:
            cur.execute(
                ...
                INSERT INTO jobs
                    (id, customer_id, title, status,
                     scheduled_start, scheduled_end, contract_value, custom_fields)
                VALUES
                    (%s, %s, %s, %s, %s, %s, %s, %s)
                """,
                [
                    j["id"], j["customer_id"], j.get("title"), j.get("status"),
                    j.get("scheduled_start"), j.get("scheduled_end"),
                    j.get("contract_value"),
                    Jsonb(j.get("custom_fields") or {}),
                ],
            )
        for job_id, items in (seed.get("notes") or {}).items():
            for n in items:
                cur.execute(
                    """
                    INSERT INTO notes (id, job_id, body, source)
                    VALUES (%s, %s, %s, %s)
                    """,
                    [n["id"], job_id, n.get("body"), n.get("source")],
                )
        for job_id, items in (seed.get("subtasks") or {}).items():
            for s in items:
                cur.execute(
                    """
                    INSERT INTO subtasks
                        (id, job_id, title, due_date, priority, status, source)
                    VALUES
                        (%s, %s, %s, %s, %s, %s, %s)
                    """,
                    [
                        s["id"], job_id, s.get("title"), s.get("due_date"),
                        s.get("priority"), s.get("status"), s.get("source"),
                    ],
                )


def reset_db():
    """TRUNCATE all four tables and re-seed. Backs the /_reset endpoint."""
    with pool.connection() as conn:
        with conn.cursor() as cur:
            # Order satisfies FK references without needing CASCADE.
            cur.execute("TRUNCATE TABLE notes, subtasks, jobs, customers")
        _seed_into_db(conn, _seed_data())
        conn.commit()


# ─────────────────────────────────────────────────────────────────────
# Row → API-shape serializers. Reconstruct the exact JSON shapes the
# API returned out of the old in-memory dict so nothing downstream
# (orchestrator, writers, React app, admin pages) sees any change.
# ─────────────────────────────────────────────────────────────────────

def _iso(value):
    """datetime/date -> ISO string. None and other types pass through."""
    if value is None:
        return None
...
def _serialize_job(row):
    ...
    return {
        ...
        "custom_fields": row["custom_fields"] if row["custom_fields"] is not None else {},
        "created_at": _iso(row["created_at"]),
        "updated_at": _iso(row["updated_at"]),
    }
...
def list_customers():
    with pool.connection() as conn, conn.cursor() as cur:
        cur.execute("SELECT * FROM customers ORDER BY created_at ASC, id ASC")
        rows = cur.fetchall()
    return paginate([_serialize_customer(r) for r in rows])
...
def get_customer(customer_id):
    ...
    return jsonify({"data": _serialize_customer(row)})
    ...
    with pool.connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
                ...
                [
                    new_id,
                    body.get("display_name", "New Customer"),
                    body.get("first_name", ""),
                    body.get("last_name", ""),
                    body.get("email", ""),
                    body.get("phone", ""),
                    Jsonb(body.get("address") or {}),
                ],
            )
            row = cur.fetchone()
        conn.commit()
    return jsonify({"data": _serialize_customer(row)}), 201

@app.patch("/customers/<customer_id>")
def update_customer(customer_id):
    body = request.get_json() or {}
    # Build an UPDATE only for keys the client actually sent — preserves the
    # "fields not in body are not touched" semantics of the old dict update.
    allowed = ("display_name", "first_name", "last_name", "email", "phone", "address")
    sets = []
    params = []
    for k in allowed:
        if k in body:
...
def delete_customer(customer_id):
    """Delete a customer and cascade-delete every job, note, and subtask
    that belonged to them. The cascade is enforced by the foreign keys'
    ON DELETE CASCADE — we just need to collect the deleted job ids for
    the response shape callers expect."""
    with pool.connection() as conn:
        with conn.cursor() as cur:
            # Check existence + collect job ids before the delete (CASCADE
            # removes them; we need them for the response).
            cur.execute("SELECT id FROM jobs WHERE customer_id = %s", [customer_id])
...
def list_jobs():
    ...
    with pool.connection() as conn, conn.cursor() as cur:
        if customer_id:
            cur.execute(
                "SELECT * FROM jobs WHERE customer_id = %s "
                "ORDER BY created_at ASC, id ASC",
                [customer_id],
            )
        else:
            cur.execute("SELECT * FROM jobs ORDER BY created_at ASC, id ASC")
        rows = cur.fetchall()
    return paginate([_serialize_job(r) for r in rows])
...
def get_job(job_id):
    ...
    return jsonify({"data": _serialize_job(row)})
        ...
        with conn.cursor() as cur:
            cur.execute(
                ...
                [
                    ...
                    body.get("title", "New Job"),
                    body.get("status", "design"),
                    body.get("scheduled_start"),
                    body.get("scheduled_end"),
                    body.get("contract_value"),
                    Jsonb(body.get("custom_fields") or {}),
                ],
            )
            row = cur.fetchone()
        conn.commit()
    return jsonify({"data": _serialize_job(row)}), 201

@app.patch("/jobs/<job_id>")
def update_job(job_id):
    body = request.get_json() or {}
    # MERGE custom_fields via JSONB || (PostgreSQL's concat-merge operator)
    # — same semantics as the old dict.update() but at the DB level.
    allowed = ("customer_id", "title", "status",
               "scheduled_start", "scheduled_end", "contract_value")
    sets = []
    params = []
    for k in allowed:
        if k in body:
    ...
    if "custom_fields" in body:
        sets.append("custom_fields = custom_fields || %s")
        params.append(Jsonb(body["custom_fields"] or {}))
    sets.append("updated_at = now()")
...
def list_notes(job_id):
    with pool.connection() as conn, conn.cursor() as cur:
        ...
        if cur.fetchone() is None:
            return jsonify({"error": "Job not found"}), 404
        cur.execute(
            "SELECT * FROM notes WHERE job_id = %s "
            "ORDER BY created_at ASC, id ASC",
            [job_id],
        )
        rows = cur.fetchall()
    return paginate([_serialize_note(r) for r in rows])
...
def add_note(job_id):
    ...
    with pool.connection() as conn:
        with conn.cursor() as cur:
            ...
            row = cur.fetchone()
        conn.commit()
    return jsonify({"data": _serialize_note(row)}), 201
...
def list_subtasks(job_id):
    with pool.connection() as conn, conn.cursor() as cur:
        ...
        if cur.fetchone() is None:
            return jsonify({"error": "Job not found"}), 404
        cur.execute(
            "SELECT * FROM subtasks WHERE job_id = %s "
            "ORDER BY created_at ASC, id ASC",
            [job_id],
        )
        rows = cur.fetchall()
    return paginate([_serialize_subtask(r) for r in rows])
...
def create_subtask(job_id):
    ...
    with pool.connection() as conn:
        with conn.cursor() as cur:
            ...
            row = cur.fetchone()
        conn.commit()
    return jsonify({"data": _serialize_subtask(row)}), 201

@app.patch("/subtasks/<subtask_id>")
def update_subtask(subtask_id):
    body = request.get_json() or {}
    allowed = ("title", "due_date", "priority", "status", "source")
    sets = []
    params = []
    for k in allowed:
        if k in body:
    ...
    with pool.connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
                f"UPDATE subtasks SET {', '.join(sets)} "
    ...
    return jsonify({"data": _serialize_subtask(row)})

# ─────────────────────────────────────────────────────────────────────
# Custom field schema (static reference data — not in the DB)
# ─────────────────────────────────────────────────────────────────────
@app.get("/custom_fields")
def list_custom_fields():
    return jsonify({"data": _seed_data()["custom_fields_schema"]})
...
@app.post("/_reset")
def reset_data():
    """Truncate every table and reseed. Not a real FieldPulse endpoint —
...
def admin_index():
    with pool.connection() as conn, conn.cursor() as cur:
        cur.execute("SELECT * FROM customers ORDER BY created_at ASC, id ASC")
        customers = [_serialize_customer(r) for r in cur.fetchall()]
        cur.execute(
            "SELECT customer_id, COUNT(*) AS n FROM jobs GROUP BY customer_id"
        )
        job_counts = {r["customer_id"]: r["n"] for r in cur.fetchall()}
    # Ensure every customer has an entry (even with zero jobs).
    for c in customers:
        job_counts.setdefault(c["id"], 0)
...
def admin_customer(customer_id):
    with pool.connection() as conn, conn.cursor() as cur:
        ...
        jobs = [_serialize_job(r) for r in cur.fetchall()]
    return render_template_string(
        _ADMIN_CUSTOMER_TPL,
        customer=_serialize_customer(cust_row),
        jobs=jobs,
        head=_ADMIN_HEAD,
        banner=_ADMIN_BANNER,
    )
...
def admin_job(job_id):
    with pool.connection() as conn, conn.cursor() as cur:
        ...
        cur.execute(
            "SELECT * FROM customers WHERE id = %s", [job_row["customer_id"]]
        )
        cust_row = cur.fetchone()
        cur.execute(
            "SELECT * FROM notes WHERE job_id = %s "
        ...
        notes = [_serialize_note(r) for r in cur.fetchall()]
        cur.execute(
            "SELECT * FROM subtasks WHERE job_id = %s "
            "ORDER BY created_at ASC, id ASC",
            [job_id],
        )
        subtasks = [_serialize_subtask(r) for r in cur.fetchall()]
    job = _serialize_job(job_row)
    customer = (
        _serialize_customer(cust_row)
        if cust_row is not None
        else {"id": job["customer_id"], "display_name": "(unknown)"}
    )
...
# ─────────────────────────────────────────────────────────────────────
# Module-level bootstrap — runs once when this module is imported, whether
# by `python mock_fieldpulse.py` (the dev server below) or by gunicorn
# (which never executes the __main__ block). This is where the connection
# pool is opened and the schema is created/seeded.
#
# Each gunicorn worker imports the module fresh (preload is off by default)
# and gets its own pool, which is what you want — pools should not be forked.
# ─────────────────────────────────────────────────────────────────────
def _bootstrap():
    pool.open()
...
if __name__ == "__main__":
    # Render-friendly: honor $PORT, default to 5000 for local dev.
    port = int(os.environ.get("PORT", 5000))
    print("─" * 60)
    print("Mock FieldPulse API (Flask dev server)")
